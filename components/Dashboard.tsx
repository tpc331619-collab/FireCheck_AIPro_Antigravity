...
const Dashboard: React.FC<DashboardProps> = ({ user, onCreateNew, onAddEquipment, onMyEquipment, onSelectReport, onLogout, onUserUpdate, onManageHierarchy, onOpenMapEditor }) => {
    ...
    useEffect(() => {
        if (!user.isGuest && user.uid) {
            // Load dashboard data...
            // (truncated for brevity in search, but logic exists here)
        }
    }, [user]);

    // Render Abnormal Recheck Logic
    ...
    const fetchNameCount = async () => {
        const hierarchy = await StorageService.getEquipmentHierarchy(user.uid);
        if (hierarchy) {
            let count = 0;
            Object.values(hierarchy).forEach(buildings => {
                Object.values(buildings).forEach(names => {
                    count += names.length;
                });
            });
            setNameCount(count);
    ...
    const fetchEquipmentStats = async () => {
        ...
        const stats = definitions.reduce((acc, curr) => {
            // Use equipmentDetail if available (to skip category prefix), otherwise fallback to name
