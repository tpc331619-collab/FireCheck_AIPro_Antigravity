...
import { X, Upload, Plus, Trash2, Save, MapPin, ZoomIn, ZoomOut, Move, RotateCw, Grid, MousePointer2, Download, Check, ArrowLeft, RefreshCcw, ChevronRight } from 'lucide-react';
...
const EquipmentMapEditor: React.FC<EquipmentMapEditorProps> = ({ user, isOpen, onClose, existingMap, initialMapId, systemSettings: propSystemSettings, isAdmin }) => {
    ...
    // Internal System Settings State
    const [systemSettings, setSystemSettings] = useState<SystemSettings>(propSystemSettings || { allowGuestView: false });

    useEffect(() => {
        if (!propSystemSettings && user?.uid) {
            StorageService.getSystemSettings().then(settings => {
                if (settings) setSystemSettings(settings);
            });
        }
    }, [propSystemSettings, user?.uid]);

    useEffect(() => {
        if (isOpen) {
            console.log("EquipmentMapEditor Mounted"); // Debug Log
            loadMaps().then((data) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-2 sm:p-4 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
            ...
            >
                ...
                {/* Header */}
                <div className="bg-white border-b border-slate-200 px-4 sm:px-6 py-3 sm:py-4 flex justify-between items-center shrink-0 shadow-sm z-30">
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-3 mt-1 text-xs text-slate-500">
                                                        ...
                                                        <span className="flex items-center gap-1">
                                                            <Save className="w-3 h-3" /> {map.size ? (map.size / 1024 / 1024).toFixed(1) + 'MB' : '-'}
                                                        </span>
                                                    </div>
                                        ...
                                        {image && (
                                            ...
                                            >
                                                <img
                                                    ...
                                                    className="block w-auto h-auto object-contain pointer-events-none select-none"
